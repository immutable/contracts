...
export async function run() {
    ...
    console.log("Deploy Create 2 deployer...");
    let create2Deployer = await deployChildContract("OwnableCreate2Deployer", reservedDeployerWallet, nonceReserved, create2Owner);
    console.log("Transaction submitted: ", JSON.stringify(create2Deployer.deployTransaction, null, 2));
    await waitForReceipt(create2Deployer.deployTransaction.hash, childProvider);
    console.log("Deployed to CHILD_CREATE2_DEPLOYER: ", create2Deployer.address);
    await verifyChildContract("OwnableCreate2Deployer", create2Deployer.address);
}
run();