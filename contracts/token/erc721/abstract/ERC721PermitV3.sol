...
import {EIP712Upgradeable} from "openzeppelin-contracts-upgradeable-4.9.3/utils/cryptography/EIP712Upgradeable.sol";
import {IERC1271Upgradeable} from "openzeppelin-contracts-upgradeable-4.9.3/interfaces/IERC1271Upgradeable.sol";
import {BytesLib} from "solidity-bytes-utils/contracts/BytesLib.sol";
import {IERC4494} from "./IERC4494.sol";
import {ERC721Upgradeable, ERC721BurnableUpgradeable, IERC165Upgradeable} from "openzeppelin-contracts-upgradeable-4.9.3/token/ERC721/extensions/ERC721BurnableUpgradeable.sol";
// Errors
...
//    function supportsInterface(bytes4 interfaceId) public view virtual override(IERC165Upgradeable, ERC721Upgradeable) returns (bool) {
    ...
    function _permit(address spender, uint256 tokenId, uint256 deadline, bytes memory sig) internal virtual {
        ...
        if (sig.length == 64) {
            // ERC2098 Sig
            recoveredSigner = ECDSAUpgradeable.recover(
                digest,
            ...
            // typical EDCSA Sig
            recoveredSigner = ECDSAUpgradeable.recover(digest, sig);
        } else {
    ...
    function _isValidERC1271Signature(address spender, bytes32 digest, bytes memory sig) private view returns (bool) {
        ...
        (bool success, bytes memory res) = spender.staticcall(
            abi.encodeWithSelector(IERC1271Upgradeable.isValidSignature.selector, digest, sig)
        );

        if (success && res.length == 32) {
            bytes4 decodedRes = abi.decode(res, (bytes4));
            if (decodedRes == IERC1271Upgradeable.isValidSignature.selector) {
                return true;
            }
        }
    ...
    uint256[50] private __ERC721PermitGap;
