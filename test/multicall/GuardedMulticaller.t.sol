...
contract GuardedMulticallerTest is Test {
    ...
    function setUp() public {
        ...
        functionPermits[0] = GuardedMulticaller.FunctionPermit({
            target: address(mock), functionSelector: MockFunctions.succeed.selector, permitted: true
        });
        functionPermits[1] = GuardedMulticaller.FunctionPermit({
            target: address(mock), functionSelector: MockFunctions.revertWithNoReason.selector, permitted: true
        });
        vm.prank(deployer);
    ...
    function test_RevertIfTargetsDataMismatch() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                GuardedMulticaller.AddressDataArrayLengthsMismatch.selector, targets.length, data.length
            )
        );
        gmc.execute(signer, ref, targets, data, deadline, signature);
    }
    ...
    function test_RevertIfFunctionDisallowed() public {
        ...
        functionPermits[0] = GuardedMulticaller.FunctionPermit({
            target: address(mock), functionSelector: MockFunctions.succeed.selector, permitted: false
        });
        vm.prank(deployer);
        gmc.setFunctionPermits(functionPermits);
    ...
    function test_RevertIfSetFunctionPermitsNonContract() public {
        GuardedMulticaller.FunctionPermit[] memory functionPermits = new GuardedMulticaller.FunctionPermit[](1);
        functionPermits[0] = GuardedMulticaller.FunctionPermit({
            target: deployer, functionSelector: MockFunctions.succeed.selector, permitted: true
        });
        vm.prank(deployer);
