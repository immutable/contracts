...
export async function deployImmutableContracts(serverSignerAddress: string): Promise<{
  ...
  const seaportConduitControllerContractFactory = await hre.ethers.getContractFactory("ConduitController");
  const seaportConduitControllerContract = await seaportConduitControllerContractFactory.deploy();
  ...
  await seaportValidatorFactory.deploy(
  ...
  const { conduit: conduitAddress } = await seaportConduitControllerContract.getConduit(conduitKey);
